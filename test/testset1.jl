...
@test size(ic_linked, 1) == 4  # 4 of 8 reports, 2 of 5 people with disease reports also had emergency presentations
@test size(view(ic_linked, ic_linked[!, :criteriaID] .== 4, :), 1) == 1  # 1 of 4 links made with criteria 4
@test size(view(ic_linked, ic_linked[!, :criteriaID] .== 5, :), 1) == 1  # 1 of 4 links made with criteria 5
@test size(view(ic_linked, ic_linked[!, :criteriaID] .== 6, :), 1) == 1  # 1 of 4 links made with criteria 6
@test size(view(ic_linked, ic_linked[!, :criteriaID] .== 7, :), 1) == 1  # 1 of 4 links made with criteria 7
...
@test in(Dict(:tablename => "spine",                   :status => "existent",                  :nrecords => 3), result)
@test in(Dict(:tablename => "emergency_presentations", :status => "linked with criteria ID 1", :nrecords => 4), result)
...
@test in(Dict(:tablename => "influenza_cases",         :status => "linked with criteria ID 4", :nrecords => 1), result)
@test in(Dict(:tablename => "influenza_cases",         :status => "linked with criteria ID 5", :nrecords => 1), result)
...
@test in(Dict(:tablename => "influenza_cases",         :status => "unlinked",                  :nrecords => 4), result)
